...
class QMixer(nn.Module):
    ...
    def forward(self, agent_qs, states):
        ...
        w1_wlu = w1_wlu.unsqueeze(1).expand(-1, 3, -1).reshape(-1, self.n_agents, self.embed_dim)
        b1_wlu = b1_wlu.unsqueeze(1).expand(-1, 3, -1).reshape(-1, 1, self.embed_dim)

        hidden_wlu = F.elu(th.bmm(agent_qs_wlu_masked, w1_wlu) + b1_wlu)

        w2_wlu = th.abs(self.hyper_w2_wlu(states))
        w2_wlu = w2_wlu.unsqueeze(1).expand(-1, 3, -1).reshape(-1, self.embed_dim, 1)
        v_wlu = self.V_wlu(states).unsqueeze(1).expand(-1, 3, -1).reshape(-1, 1, 1)
